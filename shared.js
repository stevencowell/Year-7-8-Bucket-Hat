(() => {
  ...
  if(nav&&![...nav.querySelectorAll('a')].some(link=>link.textContent.trim()==='Puzzles')){const busyWork=document.createElement('a');busyWork.href=location.pathname.includes('/modules/')?'../busy-work.html':'busy-work.html';busyWork.textContent='Puzzles';busyWork.dataset.busyWorkLink='';const activities=[...nav.querySelectorAll('a')].find(link=>link.textContent.trim()==='Activities');if(activities)activities.after(busyWork);else nav.append(busyWork);}
  if(nav&&![...nav.querySelectorAll('a')].some(link=>link.textContent.trim()==='Main Menu')){const mainMenu=document.createElement('a');mainMenu.href='https://stevencowell.github.io/Main-Page/';mainMenu.textContent='Main Menu';mainMenu.dataset.mainMenuLink='';nav.append(mainMenu);}
